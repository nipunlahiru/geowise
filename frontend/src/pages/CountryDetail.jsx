...
const CountryDetail = () => {
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (isFavorite) {
        await axios.delete(`${import.meta.env.VITE_API_URL}/api/user/favorites/${code}`);
        setFavorites(favorites.filter(fav => fav !== code));
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL}/api/user/favorites`, { countryCode: code });
