...
const CountryDetail = () => {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      if (user) {
        try {
          const response = await axios.get(`${process.env.VITE_API_URL}/api/user/favorites`);
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (isFavorite) {
        await axios.delete(`${process.env.VITE_API_URL}/api/user/favorites/${code}`);
        setFavorites(favorites.filter(fav => fav !== code));
      } else {
        await axios.post(`${process.env.VITE_API_URL}/api/user/favorites`, { countryCode: code });
